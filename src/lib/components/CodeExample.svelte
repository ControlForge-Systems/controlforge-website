		...
		try {
				...
				monacoInstance.languages.setMonarchTokensProvider('st', {
					tokenizer: {
						root: [
							...
							[
								/\b(IF|THEN|ELSE|ELSIF|END_IF|CASE|OF|END_CASE|FOR|TO|BY|DO|END_FOR|WHILE|END_WHILE|REPEAT|UNTIL|END_REPEAT)\b/i,
								'keyword'
							],
							[
								/\b(VAR|VAR_INPUT|VAR_OUTPUT|VAR_IN_OUT|VAR_GLOBAL|VAR_EXTERNAL|END_VAR)\b/i,
								'keyword'
							],
							[
								/\b(FUNCTION|FUNCTION_BLOCK|PROGRAM|END_FUNCTION|END_FUNCTION_BLOCK|END_PROGRAM)\b/i,
								'keyword'
							],
							...
							[
								/\b(BOOL|BYTE|WORD|DWORD|LWORD|SINT|USINT|INT|UINT|DINT|UDINT|LINT|ULINT|REAL|LREAL|TIME|DATE|TIME_OF_DAY|DATE_AND_TIME|STRING|WSTRING)\b/i,
								'type'
							],
			...
			resizeObserver = new ResizeObserver((entries) => {
				if (editor && entries.length > 0) {
					...
					if (
						Math.abs(width - (editor.getLayoutInfo().width || 0)) > 5 ||
...
<div class="border border-gray-200 rounded-lg overflow-hidden bg-white shadow-sm max-w-full">
	<div
		...
		style="height: {height}; width: 100%; max-width: 100%; overflow: hidden;"
	>
		<!-- Fallback content while Monaco loads -->
		{#if !editorInitialized}
			<pre class="fallback-editor">{code
					.replace(/&lt;/g, '<')
					.replace(/&gt;/g, '>')
					.replace(/&amp;/g, '&')}</pre>
		{/if}
