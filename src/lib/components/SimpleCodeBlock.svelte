<script lang="ts">
  ...
  function highlightCode(code: string): string {
    ...
    return cleanCode
      // Keywords
      .replace(/\b(IF|THEN|ELSE|ELSIF|END_IF|CASE|OF|END_CASE|FOR|TO|BY|DO|END_FOR|WHILE|END_WHILE|REPEAT|UNTIL|END_REPEAT|VAR|VAR_INPUT|VAR_OUTPUT|VAR_IN_OUT|VAR_GLOBAL|VAR_EXTERNAL|END_VAR|FUNCTION|FUNCTION_BLOCK|PROGRAM|END_FUNCTION|END_FUNCTION_BLOCK|END_PROGRAM|AND|OR|NOT|XOR|MOD|DIV|TRUE|FALSE)\b/gi, '<span class="keyword">$1</span>')
      // Data types
      .replace(/\b(BOOL|BYTE|WORD|DWORD|LWORD|SINT|USINT|INT|UINT|DINT|UDINT|LINT|ULINT|REAL|LREAL|TIME|DATE|TIME_OF_DAY|DATE_AND_TIME|STRING|WSTRING|AT|ARRAY|STRUCT|END_STRUCT)\b/gi, '<span class="type">$1</span>')
